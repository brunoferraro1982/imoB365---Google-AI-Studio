...
- Login social — paridade restante (ver changelog 2026-07-24, "Botões de login social do /signup eram stubs; Apple substituído por Facebook"): `src/routes/login.tsx` (página de login dedicada, distinta do mega menu e do `/signup`) continua sem nenhum botão social — só email/senha, com um comentário no código adiando isso pra "release futura". Os outros dois itens que estavam aqui (stub do Instagram em `signup.tsx`; ambiguidade do rótulo "Instagram" vs. Facebook) já foram resolvidos — `signup.tsx` e o mega menu agora usam o mesmo componente `SocialLoginButtons.tsx`, com Facebook e Instagram como botões distintos (mesmo provider por baixo, rótulos/ícones diferentes)
