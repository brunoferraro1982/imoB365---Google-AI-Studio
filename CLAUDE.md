...
| `supabase/migrations/20260625000001_fix_imovel_fotos_rls.sql` reaplicada em produção | Achado ao copiar manualmente 7 imóveis reais de `imob365.com.br/propriedades/` pro perfil do corretor Bruno Ferraro em produção (tarefa de conteúdo, não de código): upload de foto retornava "new row violates row-level security policy" — mesma causa-raiz já documentada em 22/07 pro bucket `tenant-branding`, mas desta vez no bucket `imovel-fotos`: `storage.objects` só tinha a policy de SELECT, sem INSERT/UPDATE/DELETE, em produção. Corrigido reaplicando as 6 policies da migration já existente no repo direto via `psql` na VPS (autorizado explicitamente pelo usuário antes de tocar em produção) |
