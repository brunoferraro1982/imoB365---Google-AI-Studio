...
- **Médio** — headers de segurança HTTP ausentes no nginx (`Strict-Transport-Security`, `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`)
- **Médio** — MFA implementado (`app.configuracoes.seguranca.tsx`, TOTP real) mas nunca exigido no login — `login.tsx` libera sessão completa sem checar `aal`/desafiar o fator, falsa sensação de segurança
- **Médio** — rate limiting de login inexistente em todo o stack (nem GoTrue, nem aplicação) — requisito já documentado na seção de Segurança deste arquivo, mas nunca implementado de fato
- **Médio** — rate limit da busca por IA (`interpretarBuscaConversacional`) usa um campo `_ip` auto-declarado pelo próprio cliente, trivialmente burlável — trocar pelo IP real da requisição via middleware
- **Médio** — Dependabot desabilitado no GitHub; fail2ban ausente no SSH (mitigado por SSH já ser só-por-chave)
...
- **Médio** — buckets `corretor-fotos` e `site-widgets` só com policy de `SELECT`, mesmo padrão dos 2 bugs reais já corrigidos em `imovel-fotos`/`tenant-branding` — auditar o código de upload desses 2 buckets
