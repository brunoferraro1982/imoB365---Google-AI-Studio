...
#### Segurança — achados da auditoria de 2026-07-24 (pendentes; os 2 críticos já foram corrigidos, ver changelog acima)

- **Alto** — `/app/financeiro` e `/app/comissoes` sem `roleGuard` nas rotas, e a policy `lanc_members_read` de `lancamentos_financeiros` só checa se o usuário é membro do tenant, sem checar o papel — qualquer membro (inclusive `atendente`, que a matriz de permissões diz que não deveria ter acesso a financeiro) vê o razão financeiro completo navegando direto por URL. `comissoes` já está correta (exige admin ou ser o corretor da comissão) — usar o mesmo padrão em `lancamentos_financeiros`
- **Alto** — `tenants_admin_update` ainda permite `broker` (não só `admin`) alterar `plano_slug`/`payment_status` do próprio tenant (achado original de 21/07, reconfirmado ativo em 24/07) — restringir a policy a `admin`/`super_admin`
- **Alto** — nenhuma rotina de backup do Postgres de produção (confirmado por evidência direta: sem crontab, sem script, `pg_dump` disponível mas nunca acionado) — agendar `pg_dump`/`pg_basebackup` com destino fora da própria VPS, prioridade máxima por ser o único achado irreversível se materializar
