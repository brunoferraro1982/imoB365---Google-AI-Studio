...
- **Baixo** — `geocodeAddress` sem validação real de entrada em runtime (único fora do padrão Zod); sem rate limit nos feeds/API pública (risco de scraping); 14 vulnerabilidades de dependências via `npm audit`, majoritariamente em tooling de build não usado em produção; `assinaturas`/`impulsionamentos` com RLS habilitado mas zero policies (bug funcional, não risco de segurança)
