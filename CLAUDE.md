...
- GoTrue enviando e-mails de auth com o template padrão (não os templates com a marca imoB365 de `src/lib/email-templates/*.tsx`, PR #67) — pra isso valer de verdade, precisa publicar versões HTML (placeholders Go template, ex. `{{ .ConfirmationURL }}`) desses templates numa URL pública e apontar `GOTRUE_MAILER_TEMPLATES_CONFIRMATION`/`_RECOVERY`/etc. pra elas no `.env` do compose da VPS. Aproveitar pra também setar `GOTRUE_MAILER_EXTERNAL_HOSTS=api.portal.imob365.com.br` (hoje ausente — gera warning benigno nos logs a cada request)
