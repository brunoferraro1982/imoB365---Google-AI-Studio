  ...
  async function load() {
    ...
    const [{ data: p }, { data: m }] = await Promise.all([
      ...
      (supabase as any)
        ...
        .select(
          "id,score,motivo,imovel:imoveis(id,titulo,slug,preco,endereco_bairro,endereco_cidade,tipo,finalidade)",
        )
        .eq("lead_id", leadId)
    ...
    <div className="rounded-xl border bg-card p-6">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">
          Preferências e match de imóveis
        </h2>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          ...
          <select
            ...
            value={pref.finalidade ?? ""}
            onChange={(e) => patch("finalidade", e.target.value || null)}
          >
        ...
        <div>
          ...
          <Input
            value={csv(pref.tipos)}
            onChange={(e) => patch("tipos", parseCsv(e.target.value))}
        ...
        <div>
          ...
          <Input
            value={csv(pref.bairros)}
            onChange={(e) => patch("bairros", parseCsv(e.target.value))}
        ...
        <div>
          ...
          <Input
            value={csv(pref.cidades)}
            onChange={(e) => patch("cidades", parseCsv(e.target.value))}
          />
        </div>
        <div>
          ...
          <Input
            type="number"
            value={pref.preco_min ?? ""}
            onChange={(e) => patch("preco_min", e.target.value ? Number(e.target.value) : null)}
          />
        </div>
        <div>
          ...
          <Input
            ...
            onChange={(e) => patch("preco_max", e.target.value ? Number(e.target.value) : null)}
          />
        </div>
        <div>
          ...
          <Input
            type="number"
            value={pref.quartos_min ?? ""}
            onChange={(e) => patch("quartos_min", e.target.value ? Number(e.target.value) : null)}
          />
        </div>
        <div>
          ...
          <Input
            type="number"
            value={pref.vagas_min ?? ""}
            onChange={(e) => patch("vagas_min", e.target.value ? Number(e.target.value) : null)}
          />
        </div>
        <div className="sm:col-span-2">
          ...
          <Textarea
            ...
            onChange={(e) => patch("observacoes", e.target.value || null)}
          />
        </div>
      </div>
      ...
      <div className="mt-6">
        ...
        {matches.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            Nenhum match calculado. Salve as preferências e clique em "Recalcular matches".
          </p>
        ) : (
            ...
            {matches.map((m) => (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <Link
                    to="/imovel/$slug"
                    params={{ slug: m.imovel?.slug ?? "" }}
                    className="hover:underline"
                  >
                    {m.imovel?.titulo}
                  </Link>
                  <span className="text-xs text-muted-foreground">
                    {m.imovel?.endereco_bairro} · R${" "}
                    {Number(m.imovel?.preco ?? 0).toLocaleString("pt-BR")}
                  </span>
                </div>
