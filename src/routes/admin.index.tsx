...
function AdminOverview() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [t, ta, tm, lf, al, em, ur] = await Promise.all([
        ...
        supabase
          .from("tenants")
          .select("id,plano_slug", { count: "exact", head: true })
          .eq("status", "active"),
        supabase
          .from("tenant_modules")
  ...
  const cards = [
    {
      label: "Imobiliárias",
      value: `${s.tenantsAtivos} / ${s.tenants}`,
      icon: Building2,
      to: "/admin/tenants",
    },
    {
      label: "Módulos ativados",
      value: String(s.modulosAtivos),
      icon: Layers,
      to: "/admin/modulos",
    },
    {
      label: "Receita do mês",
      value: formatBRL(s.receitaMes),
      icon: Banknote,
      to: "/admin/planos",
    },
    {
      label: "Eventos no mês",
      value: String(s.auditoria),
      icon: ShieldCheck,
      to: "/admin/auditoria",
    },
  ...
  return (
    <div className="p-8">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Controle global de tenants, planos, módulos e integrações.
      </p>
