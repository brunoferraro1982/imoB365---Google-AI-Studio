...
function Integracoes() {
  ...
  const [activeTab, setActiveTab] = useState<"metrics" | "architecture" | "playground" | "privileges">("architecture");
  ...
  const [users, setUsers] = useState<SimulatedUser[]>([
    { id: "u-49", name: "Lucas Silveira", email: "lucas.silveira@corretores.com", provider: "google", currentRole: "guest", createdAt: "24/05/2026 14:10", pendingRequest: true },
    { id: "u-72", name: "Mariana Costa", email: "mariana.costa@imobiliariaparceira.com.br", provider: "apple", currentRole: "guest", createdAt: "24/05/2026 15:45", pendingRequest: true },
    { id: "u-99", name: "Thiago Rocha", email: "thiago.rocha@assessoria.com.br", provider: "instagram", currentRole: "guest", createdAt: "23/05/2026 09:12", pendingRequest: false },
    { id: "u-12", name: "Clara Mendes", email: "clara.mendes@mendesimoveis.com", provider: "google", currentRole: "broker", createdAt: "22/05/2026 11:30" },
  ]);
  ...
  const handleRoleElevation = (userId: string, targetRole: "broker" | "admin") => {
    ...
    if (u) {
      ...
      toast.success(`Sucesso: Privilégios de ${u.name} modificados para ${targetRole.toUpperCase()}`);
    }
  };
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            ...
            <p className="text-xs text-blue-200/80 max-w-2xl leading-relaxed">
              Arquiteto de Identidades & Segurança de Aplicações apresenta a modelagem de mitigação contra hijacking, CSRF e tokens em cache para os provedores <strong>Google</strong>, <strong>Apple</strong> e <strong>Instagram (Meta)</strong>.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3.5 bg-black/30 border border-white/10 p-4 rounded-xl shrink-0 backdrop-blur-sm">
            ...
            <div className="text-left">
              <span className="text-[10px] block font-semibold text-blue-300">ADMIN GOVERNANCE</span>
              <span className="text-xs font-bold block text-white mt-0.5">Bruno Ferraro</span>
              <span className="text-[10px] block text-muted-foreground/90 font-mono mt-0.5">AppSec & System Super-Admin</span>
            </div>
          </div>
        </div>
      </div>
          ...
          {loading ? (
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {telemetryItems.map((i) => (
                <div key={i.label} className="rounded-xl border border-border bg-card p-5 group hover:border-primary/50 transition-colors duration-200">
                  <div className="flex items-center justify-between">
                    <span className="text-2xs uppercase tracking-wider font-extrabold text-muted-foreground/85">{i.label}</span>
        ...
        <div className="space-y-8">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            ...
            <div className="rounded-xl border border-border bg-card p-5 space-y-3.5 relative">
              <div className="flex items-center justify-between">
                ...
                <Badge className="bg-emerald-50 text-emerald-700 border-emerald-200 uppercase text-[9px]">Sem Fricção</Badge>
              </div>
              ...
              <ul className="text-2xs text-muted-foreground space-y-2 leading-relaxed font-sans">
                <li>• <strong>Protocolo:</strong> Integração nativa OAuth 2.0 PKCE via Supabase Auth API pública.</li>
                <li>• <strong>Mapeamento Automatizado:</strong> Foto de perfil de avatar e email verificados via OpenID Connect (OIDC).</li>
                <li>• <strong>Infraestrutura:</strong> Cadastro direto de chaves no Google Cloud Console com restrição estrita de origens JavaScript.</li>
              </ul>
              <div className="bg-muted p-2.5 rounded-lg border text-[10px] font-mono leading-relaxed text-foreground">
                <strong>Payload OIDC mapeado:</strong><br />
            ...
            <div className="rounded-xl border border-border bg-card p-5 space-y-3.5 relative">
              <div className="flex items-center justify-between">
                ...
                <Badge className="bg-sky-50 text-sky-700 border-sky-200 uppercase text-[9px]">Foco em Marketing</Badge>
              </div>
              <p className="text-xs text-foreground font-bold font-sans">
                Acelerador de Co-corretagem e Leads Sociais
              </p>
              <ul className="text-2xs text-muted-foreground space-y-2 leading-relaxed font-sans">
                <li>• <strong>Prática Recomendada:</strong> Conexão via Meta Developer App (utilizando permissões básicas do Instagram Graph API).</li>
                <li>• <strong>Autenticação de Corretores:</strong> Permite cruzar portfólios imobiliários com as credenciais exclusivas do perfil comercial.</li>
                <li>• <strong>Validação:</strong> Supabase revalida as credenciais com o endpoint de Graph Token da Meta a cada renovação.</li>
              </ul>
              <div className="bg-muted p-2.5 rounded-lg border text-[10px] font-mono leading-relaxed text-foreground">
                <strong>Payload Graph mapeado:</strong><br />
            ...
            <div className="rounded-xl border border-border bg-card p-5 space-y-3.5 relative">
              <div className="flex items-center justify-between">
                ...
                <Badge className="bg-indigo-50 text-indigo-750 border-indigo-200 uppercase text-[9px]">AppStore Compliance</Badge>
              </div>
              <p className="text-xs text-foreground font-bold font-sans">
                Alto Nível de Privacidade (IdToken JWS)
              </p>
              <ul className="text-2xs text-muted-foreground space-y-2 leading-relaxed font-sans">
                <li>• <strong>Necessidade:</strong> Obrigatório para futura homologação do applet mobile imob365 na Apple Store.</li>
                <li>• <strong>Criptografia:</strong> Assinatura de assertions locais com chave privada ECDSA (.p8) gerada no portal de desenvolvedor Apple.</li>
                <li>• <strong>Resolução:</strong> Suporta o modo &apos;Hide My Email&apos;, gerando caixa postal proxy segura e exclusiva para a empresa.</li>
              </ul>
              <div className="bg-muted p-2.5 rounded-lg border text-[10px] font-mono leading-relaxed text-foreground">
                <strong>Payload Token de Auditoria:</strong><br />
          ...
          <div className="rounded-xl border border-border bg-card p-6 space-y-4">
            <h3 className="text-sm font-extrabold text-foreground flex items-center gap-2">
              <ShieldCheck className="h-5 w-5 text-emerald-600" /> Fluxo de Segregação de privilégios e &quot;Quarentena de Funções&quot; (AppSec)
            </h3>
            
            <p className="text-xs text-muted-foreground leading-relaxed max-w-4xl">
              Em sistemas multi-inquilinos (multi-tenant) como a <strong>imob365</strong>, liberar a função admin automaticamente após o login social é um grave erro de segurança (<span className="text-destructive font-semibold">Critical Broken Access Control</span>). Sob a perspectiva de AppSec, implementamos o fluxo de <strong>Quarentena de Papéis</strong>:
            </p>
            ...
            <div className="bg-muted/40 border p-5 rounded-2xl flex flex-col md:flex-row items-stretch gap-4 text-center md:text-left relative">
              ...
              <div className="flex-1 p-3.5 bg-background border rounded-lg space-y-1 relative">
                <span className="bg-blue-50 text-blue-800 border-blue-200 border text-[9px] font-bold px-1.5 py-0.5 rounded uppercase block w-fit">Etapa 1</span>
                <h5 className="text-xs font-bold text-foreground">Entrada do Usuário</h5>
                <p className="text-[11px] text-muted-foreground leading-snug">
                  Usuário realiza o fluxo OAuth via Google, Apple ou Instagram de forma transparente.
                </p>
              </div>
              ...
              <div className="flex-1 p-3.5 bg-blue-500/[0.03] border border-blue-500/15 rounded-lg space-y-1">
                <span className="bg-[#e0483e] text-white text-[9px] font-bold px-1.5 py-0.5 rounded uppercase block w-fit">Etapa 2 (Sandbox)</span>
                ...
                <p className="text-[11px] text-muted-foreground leading-snug">
                  O registro do banco de dados na tabela <code className="bg-muted px-1 text-xs">public.members</code> é isolado sob a role provisória <code className="bg-muted px-1 font-mono text-xs text-[#e0483e]">&quot;guest&quot;</code> ou <code className="bg-muted px-1 font-mono text-xs text-[#e0483e]">&quot;guest_pending&quot;</code>.
                </p>
              </div>
              ...
              <div className="flex-1 p-3.5 bg-background border rounded-lg space-y-1">
                <span className="bg-emerald-50 border-emerald-200 border text-emerald-800 text-[9px] font-bold px-1.5 py-0.5 rounded uppercase block w-fit">Etapa 3</span>
                ...
                <p className="text-[11px] text-muted-foreground leading-snug font-sans">
                  Apenas o <strong>Super-Admin do Sistema imob365</strong> ou o respectivo Dono da Imobiliária pode promover o novo usuário a corretor (<code className="font-mono text-xs">&quot;broker&quot;</code>) ou administrador (<code className="font-mono text-xs">&quot;admin&quot;</code>).
                </p>
              </div>
            ...
            <div className="bg-amber-500/[0.04] border border-amber-500/15 p-4 rounded-xl flex items-start gap-3">
              ...
              <div className="space-y-1">
                <h4 className="text-xs font-bold text-amber-900 font-sans">Isolamento com RLS (Row Level Security)</h4>
                <p className="text-2xs text-amber-800 leading-relaxed font-sans">
                  Adicionalmente, as políticas RLS nativas do PostgreSQL no Supabase evitam que qualquer usuário com credencial de rede social acesse dados confidenciais de imóveis, proprietários ou comissões financeiras sem antes ter uma relação estruturada no mapeamento de <code className="bg-amber-100/50 px-1 text-xs">tenant_members</code> controlada pelos super-administradores do portal.
                </p>
              </div>
            </div>
          ...
          <div className="border border-border rounded-xl bg-card p-6 space-y-4">
            <h4 className="text-xs uppercase font-extrabold tracking-widest text-muted-foreground flex items-center gap-2">
              <FileCheck className="h-4 w-4" /> Diretrizes de Blindagem de Aplicação (OWASP MASVS / ASVS)
            </h4>

            <div className="grid gap-4 md:grid-cols-2">
              <div className="p-4 bg-muted/40 rounded-xl space-y-2 border">
                <h5 className="text-xs font-bold flex items-center gap-1.5">
                  <span className="bg-destructive/10 text-destructive text-[9px] font-bold px-1.5 py-0.5 rounded">A01:2026-Broken Auth</span>
                  Session Replay Mitigation
                </h5>
                <p className="text-2xs text-muted-foreground leading-relaxed font-sans">
                  Para mitigar roubos de tokens via XSS ou Session Hijacking, as chaves e sessões de provedores sociais gerados pelo Supabase são limpos dos parâmetros de querystrings via window.history imediatamente após a ingestão de sessão segura do frontend em <code className="bg-muted px-1 text-[10px]">main.tsx</code>. O token JWT possui TTL curto (1 hora) forçando rotações saudáveis programáticas.
                </p>
              </div>

              <div className="p-4 bg-muted/40 rounded-xl space-y-2 border">
                <h5 className="text-xs font-bold flex items-center gap-1.5 font-sans">
                  <span className="bg-indigo-50 text-indigo-800 border border-indigo-200 text-[9px] font-bold px-1.5 py-0.5 rounded">MFA REINFORCED</span>
                  Autenticação Multifator Adicional
                </h5>
                <p className="text-2xs text-muted-foreground leading-relaxed font-sans">
                  A nossa plataforma permite que após o login via Google ou Apple, o utilizador use TOTP cadastrado no respectivo painel de segurança do usuário, blindando duplamente acessos a feeds de exportação XML de corretores e comissões corporativas do conselho regional de corretores (CRECI).
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.1fr,1fr]">
          ...
          <div className="rounded-xl border border-border bg-card p-6 space-y-5">
            ...
            <p className="text-2xs text-muted-foreground leading-relaxed">
              Configure as chaves fictícias necessárias para testar as callbacks do sistema e a simulação de segurança. Em ambiente produtivo, esses dados devem ser declarados exclusivamente no painel do Supabase.
            </p>
            ...
            <div className="space-y-4 pt-1">
              ...
              <div className="space-y-2 border-b pb-4.5 border-border/60">
                <span className="text-[10px] uppercase font-black text-[#e0483e] block">Google Console</span>
              ...
              {/* APPLE INPUT */}
              <div className="space-y-2 border-b pb-4.5 border-border/60">
                <span className="text-[10px] uppercase font-black text-neutral-800 dark:text-neutral-100 block">Apple Developer Center</span>
                <div className="grid gap-3 sm:grid-cols-2">
                  <div>
                    <Label className="text-2xs text-muted-foreground">Apple Service ID / Client ID</Label>
                    <Input 
              ...
              <div className="space-y-2 pb-2">
                <span className="text-[10px] uppercase font-black text-pink-600 block">Meta Basic Displays (Instagram)</span>
                <div className="grid gap-3 sm:grid-cols-2">
                  <div>
              ...
              <div className="pt-2">
                <span className="text-[10px] font-bold text-muted-foreground block mb-2 font-sans">Simular Ativação Segura via Callback:</span>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2 max-h-[300px] overflow-y-auto font-mono text-2xs leading-relaxed text-emerald-400/95">
                {terminalLogs.map((log, index) => (
                  <div key={index} className="whitespace-pre-wrap select-text selection:bg-emerald-800 selection:text-white">
                    {log}
                  </div>
                ))}
                {isSimulating && (
      ...
      {activeTab === "privileges" && (
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-blue-500/5 to-indigo-500/5 border border-blue-500/10 rounded-xl p-5 flex gap-3 text-xs leading-relaxed text-slate-800 dark:text-blue-100">
            ...
            <div className="space-y-1">
              <strong className="block font-extrabold text-[#0c1424] dark:text-blue-200">Políticas de Alçada e Consentimento Governamental:</strong>
              <p className="text-2xs text-muted-foreground/95 leading-relaxed font-sans">
                Por design de segurança corporativa, qualquer utilizador cadastrado via conexões de identidades globais sociais entra em quarentena estéril com cargo de <code className="bg-white/45 px-1 font-mono text-xs">guest</code> (Visitante pendente). Somente um **Super-Admin da Plataforma** (nosso cockpit administrativo geral) ou o respectivo Administrador do Tenant detém chaves criptográficas para revogar a quarentena e assinar a alteração de Claims.
              </p>
            </div>
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            ...
            <div className="md:col-span-2 rounded-xl border border-border bg-card p-6 space-y-4">
              <div className="flex items-center justify-between">
                <h4 className="font-extrabold text-xs uppercase tracking-widest text-muted-foreground flex items-center gap-1.5 font-sans">
                  <UserCog className="h-4 w-4" /> Usuários Recentes via Social Login (Quarentena Ativa)
                </h4>
              ...
              <div className="space-y-3.5">
                {users.map((u) => {
                  const ProviderBadge = {
                    ...
                    apple: "bg-neutral-150 text-neutral-800 border-neutral-300 dark:bg-neutral-800 dark:text-white dark:border-neutral-550",
                  ...
                  return (
                    <div key={u.id} className="rounded-xl border border-border bg-card p-4.5 space-y-3 hover:border-primary/30 transition-all duration-150">
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <h5 className="text-xs font-bold text-foreground leading-none">{u.name}</h5>
                            <span className="text-[10px] text-muted-foreground font-mono mt-1 block">{u.email}</span>
                          </div>
                        </div>

                        <div className="flex items-center gap-1.5">
                          <Badge variant="outline" className={`text-[10px] py-0 px-2 uppercase ${ProviderBadge}`}>
                            {u.provider}
                          </Badge>
                          <Badge variant="outline" className={`text-[10px] py-0 px-2 uppercase ${RoleBadge}`}>
                            {u.currentRole}
                          </Badge>
                        </div>
                      </div>
            ...
            <div className="rounded-xl border border-border bg-card p-6 space-y-4">
              ...
              <p className="text-2xs text-muted-foreground leading-relaxed leading-relaxed font-sans">
                Eventos reais ou simulados de promoções de credenciais. Todo o histórico de elevação assina as transações no log imutável de governança.
              </p>

              <div className="space-y-2 border-t pt-2.5">
                {auditLogs.map((log, id) => (
                  <div key={id} className="p-2.5 rounded-lg border bg-muted/30 text-[10px] font-mono leading-relaxed text-foreground border-border/70 break-all">
                    {log}
                  </div>
                ))}
              </div>
