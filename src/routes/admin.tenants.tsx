...
function AdminTenants() {
  ...
  async function load() {
    ...
    try {
      ...
      const mappedUsers = (u ?? []).map((usr: any) => ({
        ...
        email_simulated: usr.email_simulated || (usr.nome ? `${usr.nome.toLowerCase().replace(/\s+/g, "")}@exemplo.com` : "usuario@exemplo.com"),
      ...
      if (tenantMode === "create") {
        ...
        const { data: newT, error: tErr } = await supabase
          ...
          .insert({
            ...
            slug,
            status: "active",
            plano_slug: user.plano_pretendido === "Free" ? "plan-free" : `plan-${user.plano_pretendido?.toLowerCase()}`
          } as any)
          .select()
          .maybeSingle();
  ...
  const filteredTenants = tenants.filter(t => t.nome.toLowerCase().includes(searchTerm.toLowerCase()) || t.slug.toLowerCase().includes(searchTerm.toLowerCase()));
  const filteredUsers = users.filter(usr => (usr.nome || "").toLowerCase().includes(searchTerm.toLowerCase()) || (usr.imobiliaria_nome || "").toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    <div className="p-8 font-sans">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Homologação de imobiliárias, corretores individuais, pagamentos recorrentes e vínculos RLS.</p>
        </div>
        ...
        <div className="flex bg-muted/65 p-1 rounded-xl w-fit border border-border">
          ...
          >
            <Users className="h-3.5 w-3.5" /> Fila de Aprovações ({users.filter(u => !u.aprovado).length})
          </button>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-card">
        {loading ? (
          <div className="p-10 text-center text-sm text-muted-foreground">Processando alterações…</div>
        ) : activeTab === "tenants" ? (
          /* TAB 1: TENANTS LIST */
                  ...
                  <tr key={t.id} className="hover:bg-muted/20 transition">
                    ...
                    <td className="px-5 py-3.5">
                      <Select value={t.plano_slug ?? ""} onValueChange={(v) => updateField(t.id, "plano_slug", v)}>
                        <SelectTrigger className="h-7 w-32 font-medium text-xxs"><SelectValue placeholder="—" /></SelectTrigger>
                        <SelectContent>
                          {plans.map((p) => <SelectItem key={p.slug} value={p.slug} className="text-xxs">Plano {p.nome}</SelectItem>)}
                        </SelectContent>
                      </Select>
                    </td>
                    <td className="px-5 py-3.5">
                      <Select value={t.status} onValueChange={(v) => updateField(t.id, "status", v)}>
                        <SelectTrigger className={`h-7 w-28 text-xxs font-bold capitalize ${t.status === "active" ? "text-green-600" : "text-amber-600"}`}><SelectValue /></SelectTrigger>
                        <SelectContent className="text-xxs font-sans">
                          {STATUSES.map((s) => (
                            <SelectItem key={s} value={s} className="capitalize text-xxs">{s}</SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                    </td>
          ...
          filteredUsers.length === 0 ? (
            <div className="p-10 text-center text-sm text-muted-foreground">Nenhum cadastro pendente ou correspondente encontrado.</div>
          ) : (
            <table className="w-full text-xs text-left">
              <thead className="bg-muted/50 text-xxs uppercase tracking-wider text-muted-foreground font-semibold border-b border-border">
                <tr>
              ...
              <tbody className="divide-y divide-border">
                {filteredUsers.map((usr) => {
                  ...
                  return (
                    <tr key={usr.id} className={`hover:bg-muted/20 transition ${!usr.aprovado ? "bg-amber-500/[0.02]" : ""}`}>
                      <td className="px-5 py-4">
                        <div className="font-bold text-foreground">{usr.nome || "Sem Nome"}</div>
                        <div className="text-xxs text-muted-foreground font-mono mt-0.5">{usr.email_simulated}</div>
                      </td>
                      <td className="px-5 py-4 capitalize font-semibold text-primary">
                        {usr.tipo_usuario}
                      </td>
                      <td className="px-5 py-4 font-bold">
                        <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xxs ${hasPlan ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-1.5 font-mono">
                          ...
                          {usr.pagamento_validado === true ? (
                            <span className="text-green-600 font-bold ml-1 text-4xs uppercase">Validado</span>
                          ) : (
                            hasPlan && <span className="text-amber-500 font-bold ml-1 text-4xs uppercase">Pendente</span>
                          )}
                        </div>
                      </td>
                      <td className="px-5 py-4">
                        {usr.tenant_id ? (
                          ...
                          <div className="font-semibold text-amber-700 text-xxs flex items-center gap-1 max-w-[140px] truncate" title={usr.imobiliaria_nome}>
      ...
      {approvalModal?.isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 animate-in fade-in duration-200">
          <div className="bg-card border border-border w-full max-w-md rounded-2xl p-6 shadow-xl relative animate-in zoom-in-95 duration-200 text-left font-sans text-sm">
            ...
            <p className="text-xxs text-muted-foreground mb-4">Aprovação do perfil comercial, planos de cota e isolamento de RLS.</p>

            <div className="space-y-4">
              <div>
                <label className="text-xxs font-bold text-muted-foreground uppercase tracking-wider block mb-1">Como tratar a imobiliária (Tenant)?</label>
                <div className="grid grid-cols-2 gap-2 p-1 rounded-xl bg-muted/60 mb-2">
                  <button
                    type="button"
                    onClick={() => setApprovalModal(prev => prev ? { ...prev, tenantMode: "create" } : null)}
                ...
                {approvalModal.tenantMode === "create" ? (
                  <div className="space-y-1">
                    <label className="text-3xs text-muted-foreground font-semibold">Nome comercial do novo Tenant corporativo:</label>
                    <Input 
                  ...
                  <div className="space-y-1">
                    <label className="text-3xs text-muted-foreground font-semibold">Selecione uma imobiliária parceira do sistema:</label>
                    <select
                    ...
                    >
                      ...
                      {tenants.map(t => (
                        <option value={t.id} key={t.id}>{t.nome} (plano: {t.plano_slug})</option>
                      ))}
                    </select>
                  </div>
                )}
              ...
              <div className="grid grid-cols-2 gap-3.5">
                <div>
                  <label className="text-xxs font-bold text-muted-foreground uppercase tracking-wider block mb-1">Qual é o papel (Role)?</label>
                  <select
                    value={approvalModal.selectedRole}
                    onChange={(e) => setApprovalModal(prev => prev ? { ...prev, selectedRole: e.target.value as any } : null)}
                ...
                <div>
                  <label className="text-xxs font-bold text-muted-foreground uppercase tracking-wider block mb-1">Pagamento Validado?</label>
                  <div className="flex items-center gap-2 mt-1.5 h-7">
                    ...
                    />
                    <label htmlFor="paymentValidCheck" className="text-xs font-semibold text-foreground">Confirmar Recebimento</label>
                  </div>
                </div>
              </div>
              ...
              <div className="text-3xs bg-primary/10 border border-primary/20 text-primary p-3 rounded-lg flex items-start gap-1.5 leading-normal">
                ...
                Alerta de LGPD-RLS de segurança: A vinculação associará este usuário ao tenant e aos limites do plano {approvalModal.user.plano_pretendido} com visibilidade segregada.
              </div>

              <div className="flex gap-2 pt-2 pb-1">
                <Button variant="outline" className="flex-1 text-xs h-9" onClick={() => setApprovalModal(null)}>Cancelar</Button>
                <Button type="button" className="flex-1 text-xs h-9" onClick={executeApproval}>Aprovar e Liberar</Button>
              </div>
            </div>
          </div>
        </div>
