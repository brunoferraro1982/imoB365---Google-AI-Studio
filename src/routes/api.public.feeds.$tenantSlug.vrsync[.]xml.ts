    ...
    handlers: {
      GET: async ({ params, request }) => {
        ...
        const { data: imoveis } = await supabaseAdmin
          .from("imoveis")
          .select("id,slug,titulo,descricao,finalidade,tipo,preco,condominio,iptu,area_util,area_total,quartos,suites,banheiros,vagas,endereco_logradouro,endereco_numero,endereco_bairro,endereco_cidade,endereco_uf,endereco_cep,latitude,longitude,caracteristicas,updated_at,codigo_interno")
          .eq("tenant_id", tenant.id)
          .eq("publicado", true)
          .eq("status", "ativo")
...
const FINALIDADE_MAP: Record<string, string> = { venda: "Sale", aluguel: "Rental", temporada: "Vacational" };
const TIPO_MAP: Record<string, string> = {
  apartamento: "Apartment", casa: "Home", cobertura: "Penthouse", terreno: "ResidentialAllotmentLand",
  comercial: "Office", galpao: "Warehouse", chacara: "FarmRanch", outro: "Home",
};
...
function buildVrsyncXml(
  ...
  const listings = imoveis.map((i) => {
    const fotos = (fotoMap.get(i.id) ?? []).slice(0, 30);
    const media = fotos
      .map((url, idx) => `<Media><MediaURL>${esc(url)}</MediaURL><MediaOrder>${idx + 1}</MediaOrder></Media>`)
      .join("");
