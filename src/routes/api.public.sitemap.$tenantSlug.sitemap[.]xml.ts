    ...
    handlers: {
      GET: async ({ params, request }) => {
        ...
        for (const i of imoveis ?? []) {
          urls.push(
            `<url><loc>${origin}/imovel/${i.slug}</loc><lastmod>${i.updated_at}</lastmod><priority>0.9</priority></url>`,
          );
        }
        for (const c of corretores ?? []) {
          urls.push(
            `<url><loc>${origin}/corretor/${c.slug}</loc><lastmod>${c.updated_at}</lastmod></url>`,
          );
        }
