...
function DominiosPage() {
  ...
  async function add() {
    ...
    const { error } = await supabase
      .from("tenant_domains")
      .insert({ tenant_id: tenantId, dominio: clean });
    ...
    <div className="space-y-8">
      <section className="rounded-xl border border-border bg-card p-6">
        ...
        <p className="mb-4 text-sm text-muted-foreground">
          Aponte um <code className="rounded bg-muted px-1">CNAME</code> para{" "}
          <code className="rounded bg-muted px-1">app.imob365.com.br</code> e adicione o registro{" "}
          <code className="rounded bg-muted px-1">TXT</code> de verificação gerado abaixo.
        </p>
                    ...
                    {!d.verificado && (
                      <div className="mt-2 text-xs text-muted-foreground">
                        ...
                        <code className="rounded bg-muted px-1">
                          imob365-verify={d.verification_token}
                        </code>{" "}
                        em <code className="rounded bg-muted px-1">_imob365.{d.dominio}</code>.
                      </div>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    {d.verificado && !d.primario && (
                      <Button variant="outline" size="sm" onClick={() => tornarPrimario(d.id)}>
                        Tornar primário
                      </Button>
                    )}
