    ...
    (async () => {
      ...
      const list: Check[] = [
        ...
        { id: "dominio", label: "Domínio próprio verificado (opcional)",
          done: domainsList.some(d => d.verificado), link: "/app/configuracoes/dominios" },
      ];
      setChecks(list);
      ...
      const emailDiag: IntegrationDiagnosis = {
        ...
        description: "Templates funcionais baseados em React Email integrados ao fluxo (Cadastros, Recuperação de senha, Magic-links, Convites e Notificações de Leads).",
      ...
      const totalPortalsConfigured = [vrConfigured, zapConfigured, wimoveisConfigured, olxConfigured].filter(Boolean).length;

      const portalDiag: IntegrationDiagnosis = {
        id: "portals",
        name: "Portais Imobiliários (Feeds por Tenant)",
        category: "Marketing",
        status: totalPortalsConfigured === 4 ? "active" : totalPortalsConfigured > 0 ? "sandbox" : "pending",
        statusText: `${totalPortalsConfigured} de 4 configurados`,
        description: "Disparo automático de anúncios via arquivos públicos de feed VRSync XML e OLX Realty.",
      ...
      const domainDiag: IntegrationDiagnosis = {
        ...
        statusText: activeDomains > 0 ? `${activeDomains} domínio(s) configurado(s)` : "Pendente de apontamento DNS",
        description: "Hospedagem e isolamento da marca sob domínio customizado da imobiliária.",
        details: activeDomains > 0 
      ...
      const mapsDiag: IntegrationDiagnosis = {
        ...
        description: "Renderização de pontos georreferenciados no mapa em tempo real e buscador de CEP / logradouros.",
        details: "Atualmente configurado com Fallback Ativo Automático e Gratuito: o mapa utiliza Leaflet integrado e as consultas de endereço em 'geocode.functions.ts' rodam via Nominatim OpenStreetMap de forma estável e segura. Se preferir usar o motor oficial do Google, é preciso declarar GOOGLE_MAPS_API_KEY.",
        icon: Activity
      };
      ...
      const waDiag: IntegrationDiagnosis = {
        ...
        description: "Geração instantânea de links de conversas com corretores embutidos na ficha do imóvel.",
        details: "O portal gera links automáticos diretos ('wa.me/numero_corretor') para click-to-chat sem custo. Disparo automatizado de mensagens de marketing ativo via servidores dedicados oficiais WhatsApp exige contratação e contrachave de API de terceiro (Z-API/ChatPro).",
        icon: Smartphone
      };
      ...
      const trackingDiag: IntegrationDiagnosis = {
        ...
        description: "Injeção de cookies e tags para Google Analytics 4, Tag Manager, Google Ads e Meta Pixel no 'TrackingPixels.tsx'.",
      ...
      const esignDiag: IntegrationDiagnosis = {
        ...
        description: "Modulo completo para emissão, despacho, assinatura e autenticação de contratos digitais de locação e venda.",
        details: "Simulador de Laboratório Sandbox do ClickSign com Hash de Auditoria SHA-256 e checklist automático de vistorias 100% integrados em 'routes/app.contratos.$id.tsx'. O disparo oficial de links de email/SMS fora do modo sandbox requer a chave de token de produção do ClickSign.",
        icon: ShieldCheck,
      ...
      const billingDiag: IntegrationDiagnosis = {
        ...
        description: "Área de upgrades, alteração de limites e cobranças por corretor integrado na plataforma.",
        details: "Área de planos (/app/contratacao) funcional em modo Sandbox com faturamento local e upgrade imediato de limites. Integração nativa com chaves produtivas do Stripe para recorrência de boleto, pix e cartão em conformidade PCI.",
        icon: CreditCard,
      ...
      const cartorioDiag: IntegrationDiagnosis = {
        ...
        description: "Pesquisa, solicitação e download de certidões atualizadas e matrículas de bens patrimoniais.",
        details: "Módulo de controle estético e visual cadastrado em 'app.cartorios.tsx'. Conexões oficiais com registradores digitais, Central de Registradores de Imóveis (ONR) são feitas sob demanda ou manualmente pelos consultores credenciados.",
        icon: Building2,
      ...
      const erpDiag: IntegrationDiagnosis = {
        ...
        description: "Envio de contas correlatas, split de faturamento e taxas condominiais para ERP de administração de terceiros.",
        details: "O controle de comissão de corretores, lançamentos de recebíveis de aluguel e vistorias é efetuado e centralizado diretamente de modo nativo dentro do ERP próprio da imob365.",
        icon: Server
      };
    ...
    <div className="max-w-4xl space-y-6 font-sans">
      ...
      <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h2 className="text-lg font-extrabold tracking-tight text-foreground">Status do Lançamento (Go-Live)</h2>
              <p className="text-xs text-muted-foreground mt-0.5">Analise dados operacionais e diagnostique o status de todas as integrações da imobiliária.</p>
            </div>
          </div>
        ...
        {activeTab === "checklist" && (
          <div className="mt-6 border-t border-border/65 pt-4">
            ...
            <div className="mt-2 flex items-center justify-between text-xs font-semibold text-muted-foreground font-sans">
              <span>{done} de {total} itens operacionais obrigatórios concluídos</span>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-10 space-x-2">
          ...
          <span className="text-xs text-muted-foreground font-medium pl-1.5">Auditando banco de dados e apurando chaves...</span>
        </div>
      ) : activeTab === "checklist" ? (
        <div className="space-y-3">
          {checks.map((c) => (
            <div key={c.id} className="flex items-center justify-between gap-4 rounded-xl border border-border bg-card p-4 hover:border-border/80 transition-colors">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className={`text-xs font-bold font-sans ${c.done ? "text-foreground" : "text-muted-foreground"}`}>
                    {c.label}
                  </span>
                  {c.id === "dominio" && (
                    <span className="text-[10px] block text-muted-foreground mt-0.5 font-sans">Permite hospedar no link oficial da sua imobiliária (ex: portal.imobiliaria.com)</span>
                  )}
                  {c.id === "tenant" && (
                    <span className="text-[10px] block text-muted-foreground mt-0.5 font-sans">Dados necessários para emissão de Notas Fiscais, recibos e relatórios de comissões.</span>
                  )}
                </div>
              </div>
              {c.link && (
                <Link to={c.link} className="text-xs text-primary font-extrabold hover:underline whitespace-nowrap bg-muted/60 px-3 py-1.5 rounded-lg border border-border/40 hover:bg-muted">
          ...
          {ready && (
            <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-6 text-center animate-fade-in">
              ...
              <h4 className="mt-3 text-sm font-extrabold text-emerald-800">Pronto para lançar seu Portal!</h4>
              <p className="mt-1 text-xs text-emerald-600 max-w-md mx-auto leading-relaxed">
                As configurações operacionais mínimas estão prontas. Você já pode divulgar o link do seu site para o mercado captar leads e alugar imóveis.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-amber-500/[0.04] border border-amber-500/10 rounded-xl p-4 flex gap-3 text-xs leading-relaxed text-amber-800">
            ...
            <div>
              ...
              As chaves e tokens de serviços de produção (como e-mails oficiais de domínios, assinaturas reais Kliksign e Stripe produtivo) não estão inseridas no arquivo .env global por questões de segurança. A plataforma utiliza um motor inteligente de sandbox, simulação ou fallbacks grátis para garantir que você continue utilizando a ferramenta normalmente.
            </div>
          </div>
          ...
          <div className="grid gap-4.5">
            {diagnostics.map((diag) => {
              ...
              return (
                <div key={diag.id} className="rounded-xl border border-border bg-card p-5 space-y-3.5 hover:shadow-xs transition-shadow">
                  <div className="flex flex-wrap items-center justify-between gap-2.5">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <span className="text-[10px] uppercase font-extrabold tracking-wider text-muted-foreground block leading-none">{diag.category}</span>
                        <h4 className="text-sm font-extrabold text-foreground mt-1 leading-tight">{diag.name}</h4>
                      </div>
                    </div>

                    <Badge variant="outline" className={`text-xs font-bold uppercase rounded-md px-2.5 py-0.5 border ${BadgeColor}`}>
                      {diag.statusText}
                    </Badge>
                  </div>
                  ...
                  <div className="text-xs space-y-2">
                    ...
                    <div className="bg-muted/40 p-3 rounded-lg border border-border/40 text-[11px] leading-relaxed text-foreground/90 font-mono">
                      <strong className="text-foreground block text-[10px] uppercase font-bold tracking-wide font-sans mb-1 text-muted-foreground">Diagnóstico Técnico:</strong>
                      {diag.details}
                    </div>
                  </div>
