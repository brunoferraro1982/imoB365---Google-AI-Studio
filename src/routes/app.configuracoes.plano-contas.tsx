...
function PlanoContas() {
  ...
  async function del(id: string) {
    await (supabase as any).from("plano_contas").delete().eq("id", id);
    load();
  }
    ...
    <div className="mx-auto max-w-3xl space-y-6 p-8">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Categorize receitas e despesas para relatórios contábeis.
        </p>
      </div>
            ...
            {rows.map((r) => (
              <tr key={r.id} className="border-t">
                ...
                <td className="text-center">
                  <span
                    className={`rounded px-2 py-0.5 text-xs ${r.tipo === "receita" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                  >
                    {r.tipo}
                  </span>
                </td>
