...
function ScoringPage() {
  ...
  async function add() {
    ...
    const { error } = await (supabase as any)
      .from("lead_scoring_regras")
      .insert({ tenant_id: tenantId, evento, pontos });
  ...
  return (
    <div className="max-w-3xl space-y-6 p-8">
      ...
      <p className="text-sm text-muted-foreground">
        Atribua pontos a eventos do lead. O score acumulado aparece na ficha e ajuda a priorizar
        follow-ups.
      </p>
      <div className="rounded-xl border bg-card p-4">
        ...
        <ul className="mt-4 space-y-1">
          ...
          {regras.length === 0 && (
            <li className="text-sm text-muted-foreground">Nenhuma regra cadastrada.</li>
          )}
        </ul>
      </div>
    </div>
