...
function SegurancaPage() {
  ...
  async function enroll() {
    setEnrolling(true);
    const { data, error } = await supabase.auth.mfa.enroll({ factorType: "totp", friendlyName: "App Authenticator" });
    setEnrolling(false);
  ...
  async function verify() {
    ...
    if (ch.error) { setVerifying(false); return toast.error(ch.error.message); }
    const v = await supabase.auth.mfa.verify({ factorId: qr.factorId, challengeId: ch.data.id, code });
    setVerifying(false);
    if (v.error) return toast.error(v.error.message);
    toast.success("MFA ativado");
    setQr(null); setCode(""); load();
  }
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Use um aplicativo autenticador (Google Authenticator, 1Password, Authy) para gerar um código de 6 dígitos a cada login.
            </p>
          </div>
        </div>
        ...
        <div className="mt-6">
            ...
            <ul className="space-y-2">
              {factors.map((f) => (
                <li key={f.id} className="flex items-center justify-between rounded-lg border p-3 text-sm">
                  <div>
                    <div className="font-medium">{f.friendly_name ?? f.factor_type.toUpperCase()}</div>
          ...
          {!qr && (
            <Button className="mt-4" onClick={enroll} disabled={enrolling}>
              <KeyRound className="mr-2 h-4 w-4" /> {enrolling ? "Gerando…" : "Adicionar autenticador TOTP"}
            </Button>
          )}
          ...
          {qr && (
            <div className="mt-6 rounded-lg border p-4">
              ...
              <p className="mt-2 text-xs text-muted-foreground">Ou use o segredo: <code className="rounded bg-muted px-1">{qr.secret}</code></p>
              <div className="mt-4 flex items-end gap-2">
                ...
                <Button variant="ghost" onClick={() => { setQr(null); setCode(""); }}>Cancelar</Button>
              </div>
            </div>
          )}
        </div>
