...
export const Route = createFileRoute("/app/financeiro/centros-custo")({
...
function CentrosCusto() {
  ...
  async function add() {
    ...
    const { error } = await (supabase as any)
      .from("centros_custo")
      .insert({ tenant_id: tenantId, ...novo });
    if (error) return toast.error(error.message);
    setNovo({ codigo: "", nome: "" });
    load();
  }
  async function del(id: string) {
    await (supabase as any).from("centros_custo").delete().eq("id", id);
    load();
  }
    ...
    <div className="mx-auto max-w-3xl space-y-6 p-8">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Agrupe lançamentos por filial, equipe ou projeto.
        </p>
      </div>
      <section className="rounded-xl border bg-card p-6">
        <div className="mb-3 grid gap-2 sm:grid-cols-[100px,1fr,auto]">
          ...
          />
          <Input
            placeholder="Nome (ex: Matriz, Filial SP)"
            value={novo.nome}
        ...
        <ul className="space-y-1 text-sm">
          {rows.map((r) => (
            <li key={r.id} className="flex items-center justify-between rounded border px-3 py-2">
              <span>
                <span className="mr-2 font-mono text-xs text-muted-foreground">{r.codigo}</span>
                {r.nome}
              </span>
          ...
          {rows.length === 0 && (
            <p className="py-3 text-center text-muted-foreground">Nenhum centro de custo.</p>
          )}
        </ul>
      </section>
    </div>
