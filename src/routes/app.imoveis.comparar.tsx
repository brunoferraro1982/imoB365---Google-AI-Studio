...
function CompararPage() {
  ...
  const [opcoes, setOpcoes] = useState<{ id: string; titulo: string; codigo_interno: string | null }[]>([]);
  ...
  useEffect(() => {
    (async () => {
      const q = supabase.from("imoveis").select("id,titulo,codigo_interno").order("updated_at", { ascending: false }).limit(20);
      const { data } = busca.trim()
        ? await q.ilike("titulo", `%${busca.trim()}%`)
        : await q;
      setOpcoes(data ?? []);
    })();
  }, [busca]);

  function add(id: string) { setIds([...selectedIds, id]); }
  function remove(id: string) { setIds(selectedIds.filter((i: string) => i !== id)); }
  ...
  const rows: { label: string; get: (i: Imovel) => any; fmt?: (v: any) => string }[] = [
    ...
    { label: "Cidade", get: (i) => i.endereco_cidade ? `${i.endereco_cidade}/${i.endereco_uf ?? ""}` : "—" },
    { label: "Bairro", get: (i) => i.endereco_bairro ?? "—" },
    { label: "Aceita permuta", get: (i) => i.aceita_permuta, fmt: (v) => v ? "Sim" : "Não" },
    { label: "Aceita financiamento", get: (i) => i.aceita_financiamento, fmt: (v) => v ? "Sim" : "Não" },
  ];
  ...
  function badgesFor(im: Imovel) {
    ...
    if (quartos.length > 1 && Number(im.quartos) === Math.max(...quartos)) tags.push("Mais quartos");
    return tags;
  }
    ...
    <div className="mx-auto max-w-7xl p-8">
      <div className="mb-4">
        <Button variant="ghost" size="sm" asChild>
          <Link to="/app/imoveis"><ArrowLeft className="mr-1 h-4 w-4" /> Voltar para imóveis</Link>
        </Button>
      </div>
      ...
      <div className="mt-6 rounded-xl border border-border bg-card p-4">
        <div className="flex items-center gap-3">
          <Input placeholder="Buscar imóvel para adicionar…" value={busca} onChange={(e) => setBusca(e.target.value)} className="max-w-md" />
        ...
        {busca.trim() && (
          <ul className="mt-3 max-h-48 overflow-y-auto rounded-lg border text-sm">
            {opcoes.map((o) => (
              <li key={o.id} className="flex items-center justify-between border-b px-3 py-2 last:border-0">
                <span>{o.titulo} {o.codigo_interno && <span className="text-xs text-muted-foreground">#{o.codigo_interno}</span>}</span>
                <Button size="sm" variant="outline" disabled={selectedIds.includes(o.id) || selectedIds.length >= 4} onClick={() => add(o.id)}>
                  Adicionar
                </Button>
              </li>
            ))}
      ...
      {loading ? (
        ...
        <p className="mt-8 text-sm text-muted-foreground">Selecione imóveis acima para começar a comparação.</p>
      ) : (
        <div className="mt-6 overflow-x-auto rounded-xl border border-border bg-card">
          <table className="w-full min-w-[640px] text-sm">
            <thead>
              <tr className="border-b">
                <th className="px-4 py-3 text-left text-xs uppercase text-muted-foreground">Característica</th>
                {imoveis.map((im) => (
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        <div className="mt-1 flex flex-wrap gap-1">
                          {badgesFor(im).map((b) => (
                            <Badge key={b} className="bg-emerald-600 text-[10px]">{b}</Badge>
                          ))}
                        </div>
                      </div>
                      <Button size="sm" variant="ghost" onClick={() => remove(im.id)}><X className="h-4 w-4" /></Button>
                    </div>
                  </th>
                ))}
              </tr>
              ...
              {rows.map((r) => (
                <tr key={r.label} className="border-t">
                  ...
                  {imoveis.map((im) => {
                    ...
                    return <td key={im.id} className="px-4 py-2">{str}</td>;
                  })}
                </tr>
              ))}
              <tr className="border-t">
                <td className="px-4 py-2 text-xs uppercase text-muted-foreground">Características</td>
                {imoveis.map((im) => {
                      ...
                      <ul className="space-y-1 text-xs">
                        {all.map((c) => (
                          <li key={c} className="flex items-center gap-1">
                            ...
                            <span className={(im.caracteristicas ?? []).includes(c) ? "" : "text-muted-foreground/60"}>{c}</span>
                          </li>
                        ))}
                      </ul>
                    </td>
