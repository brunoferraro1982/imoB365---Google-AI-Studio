  ...
  return (
    <div className="mx-auto max-w-3xl p-8">
      ...
      <p className="mb-8 text-sm text-muted-foreground">
        Leads recebidos pelo site são distribuídos automaticamente entre os corretores ativos da
        imobiliária.
      </p>
      ...
      <section className="mt-6 rounded-xl border border-border bg-card p-6">
        <h2 className="text-base font-semibold">
          Fila atual ({ativos.length} {ativos.length === 1 ? "corretor ativo" : "corretores ativos"}
          )
        </h2>
        {ativos.length === 0 ? (
          <p className="mt-3 text-sm text-muted-foreground">
            Nenhum corretor ativo.{" "}
            <Link to="/app/corretores" className="text-primary hover:underline">
          ...
          <ol className="mt-3 space-y-2 text-sm">
            {ativos.map((c) => (
              <li
                key={c.id}
                className={`flex items-center justify-between rounded-md border border-border px-3 py-2 ${last === c.id ? "bg-primary/5" : ""}`}
              >
                <span>{c.nome}</span>
