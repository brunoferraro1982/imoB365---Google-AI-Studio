...
function CompararPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data } = await supabase
        .from("imoveis")
        .select("id,slug,titulo,finalidade,tipo,preco,condominio,iptu,quartos,suites,banheiros,vagas,area_util,area_total,endereco_cidade,endereco_uf,endereco_bairro,caracteristicas,aceita_financiamento,aceita_permuta,imovel_fotos(storage_path,capa,ordem)")
        .in("id", ids)
        .eq("publicado", true)
        .eq("status", "ativo");
      ...
      const mapped = (data ?? [])
        ...
        .sort((a: any, b: any) => ((order.get(a.id) ?? 0) as number) - ((order.get(b.id) ?? 0) as number));
      setItems(mapped);
      setLoading(false);
    })();
  }, [ids.join(",")]);
  ...
  const linhas: { label: string; render: (i: any) => React.ReactNode }[] = [
    { label: "Preço", render: (i) => <span className="font-semibold text-primary">{formatBRL(i.preco)}</span> },
    ...
    { label: "Permuta", render: (i) => i.aceita_permuta ? "Sim" : "Não" },
    {
      ...
      render: (i) => (i.caracteristicas?.length ? (
        ...
        <div className="mt-8">
          {loading ? (
            ...
            <div className="rounded-xl border border-dashed border-border bg-card p-16 text-center">
              <p className="text-sm text-muted-foreground">Nenhum imóvel selecionado. Vá em buscar e marque a opção "Comparar".</p>
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[700px] border-separate border-spacing-0">
                <thead>
                  <tr>
                    <th className="sticky left-0 bg-background p-3 text-left text-xs font-medium text-muted-foreground" style={{ width: 160 }}></th>
                    {items.map((i: any) => (
                      <th key={i.id} className="p-3 align-top">
                        <div className="relative overflow-hidden rounded-lg border border-border bg-card">
                          <button type="button" onClick={() => remove(i.id)} aria-label="Remover" className="absolute right-2 top-2 z-10 inline-flex h-7 w-7 items-center justify-center rounded-full bg-background/90 backdrop-blur hover:bg-background">
                            <X className="h-3.5 w-3.5" />
                          </button>
                          <Link to="/imovel/$slug" params={{ slug: i.slug }}>
                            <div className="aspect-[4/3] w-full overflow-hidden bg-muted">
                              {i.capa ? (
                                <img src={publicUrl(i.capa)!} alt={i.titulo} className="h-full w-full object-cover" />
                              ) : (
                                <div className="flex h-full items-center justify-center text-xs text-muted-foreground">Sem foto</div>
                              )}
                            </div>
                            <div className="p-3 text-left">
                              <h3 className="line-clamp-2 text-sm font-semibold leading-tight">{i.titulo}</h3>
                ...
                <tbody>
                  {linhas.map((l) => (
                    <tr key={l.label} className="border-t border-border">
                      <td className="sticky left-0 bg-background p-3 align-top text-xs font-medium text-muted-foreground">{l.label}</td>
                      {items.map((i: any) => (
                        <td key={i.id} className="border-l border-border p-3 align-top text-sm">{l.render(i)}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
