...
import { Bookmark, Search as SearchIcon, Trash2, Sparkles, AlertCircle, RefreshCw, CheckCircle2 } from "lucide-react";
...
function calcularMatch(imovel: any, filtros: Record<string, any>) {
  ...
  // Finalidade: peso 20
  totalWeight += 20;
  if (!filtros.finalidade || filtros.finalidade === "todos" || imovel.finalidade === filtros.finalidade) {
    score += 20;
...
function MinhasBuscasPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase
        .from("imoveis")
        .select("id, slug, titulo, preco, finalidade, tipo, quartos, banheiros, vagas, area_util, endereco_bairro, endereco_cidade, imovel_fotos(storage_path, capa, ordem)")
        .eq("publicado", true)
        .eq("status", "ativo")
    ])
      .then(([r, { data }]) => {
  ...
  const handleSimulateSync = () => {
    ...
    setTimeout(() => {
      ...
      toast.success("Fila de buscas processada com sucesso! 0 e-mails de alerta pendentes na fila.");
    }, 1500);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="-mt-3 text-sm text-muted-foreground">
        Receba alertas automáticos por e-mail quando novos imóveis de proprietários ou corretores baterem com suas preferências.
      </p>
      ...
      <div className="mt-6">
        {loading ? (
          ...
          <div className="rounded-xl border border-dashed border-border bg-card p-16 text-center">
            <p className="text-sm text-muted-foreground">Você ainda não salvou nenhuma busca no catálogo.</p>
                ...
                <div key={b.id} className="rounded-xl border border-border bg-card overflow-hidden">
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {matchesList.length > 0 && (
                            <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-2 py-0.5 text-[10px] font-bold text-emerald-600 dark:text-emerald-400">
                              <Sparkles className="h-2.5 w-2.5" /> {matchesList.length} Match{matchesList.length !== 1 && "es"}
                            </span>
                          )}
                        </div>
                        <p className="mt-1.5 text-xs text-muted-foreground font-mono">{resumo(b.filtros)}</p>
                        {b.ultimo_envio && (
                          <p className="mt-1 text-[11px] text-muted-foreground opacity-90">Último disparo da fila: {new Date(b.ultimo_envio).toLocaleString("pt-BR")}</p>
                        )}
                      </div>
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        <Link to="/buscar" search={searchParams(b.filtros)}>
                          <Button size="sm" variant="outline"><SearchIcon className="mr-1.5 h-4 w-4" />Abrir busca</Button>
                        </Link>
                        {matchesList.length > 0 && (
                        ...
                        <Button size="sm" variant="ghost" onClick={() => handleRemove(b.id)} className="text-destructive hover:bg-destructive/10 hover:text-destructive">
                  ...
                  {isExpanded && matchesList.length > 0 && (
                    <div className="bg-muted/30 border-t border-border px-5 py-4 space-y-3">
                      <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground">🎯 Sugestões do Match Inteligente</h4>
                      <div className="grid gap-3 sm:grid-cols-2">
                        {matchesList.slice(0, 4).map(({ imovel, calc }) => (
                          <div key={imovel.id} className="flex gap-3 bg-card border border-border p-3 rounded-lg overflow-hidden shrink-0">
                            <div className="h-16 w-16 bg-muted rounded-md overflow-hidden relative flex-shrink-0">
                              {imovel.capa ? (
                                <img src={publicUrl(imovel.capa)!} alt="" className="h-full w-full object-cover" />
                              ) : (
                            ...
                            <div className="min-w-0 flex-1 flex flex-col justify-between">
                              <div>
                                <Link to="/imovel/$slug" params={{ slug: imovel.slug }} className="block hover:underline">
                                  <h5 className="font-bold text-xs truncate leading-tight">{imovel.titulo}</h5>
                                </Link>
                                <p className="text-[10px] text-muted-foreground truncate">{imovel.endereco_bairro} {imovel.endereco_cidade && `· ${imovel.endereco_cidade}`}</p>
                              </div>
                              <div className="flex items-center justify-between mt-1">
                                <span className="text-xs font-semibold text-primary">{formatBRL(imovel.preco)}</span>
