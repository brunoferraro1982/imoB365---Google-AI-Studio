...
const STATUS_INFO: Record<string, { label: string; icon: any; cls: string }> = {
  agendada: {
    label: "Agendada",
    icon: Clock,
    cls: "text-amber-700 bg-amber-100 dark:text-amber-200 dark:bg-amber-900/40",
  },
  confirmada: {
    label: "Confirmada",
    icon: CheckCircle2,
    cls: "text-blue-700 bg-blue-100 dark:text-blue-200 dark:bg-blue-900/40",
  },
  realizada: {
    label: "Realizada",
    icon: CheckCircle2,
    cls: "text-emerald-700 bg-emerald-100 dark:text-emerald-200 dark:bg-emerald-900/40",
  },
  cancelada: {
    label: "Cancelada",
    icon: XCircle,
    cls: "text-red-700 bg-red-100 dark:text-red-200 dark:bg-red-900/40",
  },
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        Visitas que você solicitou pelos sites das imobiliárias.
      </p>
                ...
                <div key={v.id} className="rounded-xl border border-border bg-card p-5">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <span
                          className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ${info.cls}`}
                        >
                      ...
                      {v.nps_score != null && (
                        <p className="mt-2 inline-flex items-center gap-1 text-xs text-amber-700">
                          <Star className="h-3 w-3 fill-amber-500 text-amber-500" /> Você avaliou:{" "}
                          {v.nps_score}/10
                        </p>
                      )}
                    </div>
