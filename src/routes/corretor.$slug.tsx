    ...
    (async () => {
      ...
      if (imoveisList.length) {
        const { data: fotos } = await supabase
          ...
          .in("imovel_id", imoveisList.map((i) => i.id))
          .eq("capa", true);
        const map: Record<string, string> = {};
        (fotos ?? []).forEach((f: { imovel_id: string; storage_path: string }) => {
          map[f.imovel_id] = supabase.storage.from("imovel-fotos").getPublicUrl(f.storage_path).data.publicUrl;
        });
        setCapas(map);
      }
      setLoading(false);
      ...
      <main className="mx-auto max-w-6xl px-6 py-10">
        <div className="grid gap-10 lg:grid-cols-[280px_1fr]">
          <aside className="h-fit rounded-xl border border-border bg-card p-6 text-center">
            {corretor.foto_url ? (
              <img src={corretor.foto_url} alt={corretor.nome} className="mx-auto h-32 w-32 rounded-full object-cover" />
            ) : (
            ...
            {corretor.creci && (
              <p className="mt-2 flex items-center justify-center gap-1 text-xs text-muted-foreground">
                <BadgeCheck className="h-3.5 w-3.5 text-primary" /> CRECI {corretor.creci}{corretor.creci_uf ? "/" + corretor.creci_uf : ""}
              </p>
            )}
            <div className="mt-6 space-y-2 text-left text-sm">
              {corretor.whatsapp && (
                <a href={waLink(corretor.whatsapp)} target="_blank" rel="noreferrer" className="flex items-center gap-2 rounded-md bg-primary px-3 py-2 font-medium text-primary-foreground hover:opacity-90">
                  <MessageCircle className="h-4 w-4" /> WhatsApp
                </a>
              )}
              {corretor.telefone && (
                <a href={`tel:${corretor.telefone}`} className="flex items-center gap-2 rounded-md border border-border px-3 py-2 hover:bg-muted">
                  <Phone className="h-4 w-4" /> {corretor.telefone}
                </a>
              )}
              {corretor.email && (
                <a href={`mailto:${corretor.email}`} className="flex items-center gap-2 rounded-md border border-border px-3 py-2 hover:bg-muted">
            ...
            <section>
              ...
              {imoveis.length === 0 ? (
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  {imoveis.map((i) => (
                    <Link key={i.id} to="/imovel/$slug" params={{ slug: i.slug }} className="group overflow-hidden rounded-xl border border-border bg-card transition-shadow hover:shadow-md">
                      <div className="aspect-[4/3] bg-muted">
                        {capas[i.id] && <img src={capas[i.id]} alt={i.titulo} className="h-full w-full object-cover transition-transform group-hover:scale-105" />}
                      </div>
                      <div className="p-4">
                        <p className="text-xs uppercase tracking-wide text-primary">{FINALIDADE_LABEL[i.finalidade]} · {TIPO_LABEL[i.tipo]}</p>
                        <h3 className="mt-1 line-clamp-1 font-semibold">{i.titulo}</h3>
                        {i.endereco_cidade && (
                          <p className="mt-1 flex items-center gap-1 text-xs text-muted-foreground">
                            <MapPin className="h-3 w-3" /> {i.endereco_cidade}{i.endereco_uf ? "/" + i.endereco_uf : ""}
                          </p>
                        )}
