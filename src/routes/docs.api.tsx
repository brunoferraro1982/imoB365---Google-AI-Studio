...
export const Route = createFileRoute("/docs/api")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Documentação da API REST pública da imob365: endpoints de imóveis, leads e webhooks." },
    ],
  }),
...
function Endpoint({ method, path, children }: { method: string; path: string; children: React.ReactNode }) {
  const color = method === "GET" ? "bg-emerald-600" : method === "POST" ? "bg-blue-600" : "bg-amber-600";
  return (
    <section className="mt-8 rounded-xl border border-border bg-card p-6">
      <div className="flex items-center gap-3">
        <span className={`rounded-md ${color} px-2 py-0.5 font-mono text-xs font-bold text-white`}>{method}</span>
...
function ApiDocsPage() {
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-3xl p-8">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">REST · JSON · autenticação por chave de API</p>
        </div>
        </header>
      ...
      <section className="mt-8 space-y-3 text-sm">
        ...
        <p className="text-muted-foreground">
          Envie sua chave no header <code className="rounded bg-muted px-1.5 py-0.5 text-xs">X-Api-Key</code> em todas as
  ...
  -H "X-Api-Key: <SUA_CHAVE>"`}</Code>
      ...
      <Endpoint method="GET" path="/api/public/v1/imoveis">
        ...
        <ul className="list-disc pl-5">
          <li><code>tipo</code> — apartamento, casa, comercial, terreno…</li>
          <li><code>finalidade</code> — venda, locacao</li>
          <li><code>cidade</code>, <code>uf</code></li>
          <li><code>preco_min</code>, <code>preco_max</code></li>
          <li><code>limit</code> (padrão 20, máx 100)</li>
        </ul>
        <Code>{`{
  "items": [
    {
  ...
  "imovel_id": "uuid (opcional)"
        ...
        <p>Resposta <strong>201</strong>:</p>
      ...
      <section className="mt-12 space-y-3 text-sm">
        ...
        <p className="text-muted-foreground">
          Configure URLs em <strong>Configurações → Webhooks</strong> para receber eventos: <code>lead.created</code>,
          <code> lead.atribuido</code>, <code>lead.convertido</code>, <code>imovel.publicado</code>,
          <code> contrato.assinado</code>, <code>contrato.ativo</code>.
        </p>
        <p className="text-muted-foreground">
          Cada entrega é assinada via header <code>X-Imob365-Signature</code> com HMAC-SHA256 do corpo usando o segredo do webhook.
        </p>
      </section>
