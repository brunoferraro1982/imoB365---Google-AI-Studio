    ...
    (async () => {
      try {
        ...
        if (error || !data || data.length === 0) {
          ...
          const hasFree = data.some(p => p.preco_mensal === 0 || p.nome.toLowerCase().includes("grátis") || p.nome.toLowerCase().includes("free") || p.nome.toLowerCase().includes("starter"));
          if (!hasFree) {
            const freePlan: Plan = {
              id: "plan-free",
              nome: "Grátis",
            ...
            return (
              ...
              >
                ...
                <div className="mt-3">
                  ...
                  {p.preco_mensal > 0 && <span className="text-sm text-muted-foreground">/mês</span>}
                </div>
                <ul className="mt-5 space-y-2 text-xs text-muted-foreground">
                  <li className="flex items-start gap-2"><Check className="mt-0.5 h-3.5 w-3.5 shrink-0 text-primary" /> Até {p.limites?.imoveis?.toLocaleString("pt-BR")} imóveis</li>
                  <li className="flex items-start gap-2"><Check className="mt-0.5 h-3.5 w-3.5 shrink-0 text-primary" /> {p.limites?.usuarios} usuários</li>
                  <li className="flex items-start gap-2"><Check className="mt-0.5 h-3.5 w-3.5 shrink-0 text-primary" /> {cotaText}</li>
                  {highlights.map((h) => (
                    <li key={h} className="flex items-start gap-2"><Check className="mt-0.5 h-3.5 w-3.5 shrink-0 text-primary" /> {h}</li>
                  ))}
                </ul>
