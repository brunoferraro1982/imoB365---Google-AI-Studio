...
function TenantHome() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!tenant) { setNotFoundState(true); setLoading(false); return; }
      const [{ data: cfg }, { data: pages }, { data: imv }] = await Promise.all([
        ...
        supabase.from("imoveis").select("id,slug,titulo,tipo,finalidade,preco,quartos,banheiros,area_util,endereco_bairro,endereco_cidade,endereco_uf").eq("tenant_id", tenant.id).eq("publicado", true).eq("status", "ativo").order("publicado_em", { ascending: false }).limit(12),
      ]);
      if (!cfg) { setNotFoundState(true); setLoading(false); return; }
      setCtx({
        tenantSlug: tenant.slug, tenantNome: tenant.nome,
        settings: cfg, pages: (pages ?? []) as any,
      });
      setHero(cfg);
      setSobre(cfg.sobre_html ?? "");
      setImoveis(imv ?? []);
      if (imv && imv.length) {
        const { data: fotos } = await supabase.from("imovel_fotos").select("imovel_id,storage_path,capa,ordem").in("imovel_id", imv.map((i) => i.id)).order("capa", { ascending: false }).order("ordem");
        const map: Record<string, string> = {};
        (fotos ?? []).forEach((f) => { if (!map[f.imovel_id]) map[f.imovel_id] = photoUrl(f.storage_path); });
        setFotosMap(map);
      }
      setLoading(false);
    })();
    ...
    <TenantSiteLayout ctx={ctx}>
      ...
      <section className="relative overflow-hidden border-b border-border">
        ...
        <div className="mx-auto max-w-6xl px-6 py-20 text-center md:py-28">
          ...
          {hero.hero_subtitulo && (
            <p className="mx-auto mt-4 max-w-2xl text-lg text-muted-foreground">{hero.hero_subtitulo}</p>
          )}
          <div className="mt-8">
            <a href="#imoveis"><Button size="lg">{hero.hero_cta_label || "Ver imóveis"}</Button></a>
          </div>
        </div>
      </section>
      ...
      <section id="imoveis" className="mx-auto max-w-6xl px-6 py-16">
        ...
        {imoveis.length === 0 ? (
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {imoveis.map((i) => (
              <Link key={i.id} to="/imovel/$slug" params={{ slug: i.slug }} className="group overflow-hidden rounded-xl border border-border bg-card transition hover:shadow-lg">
                <div className="aspect-[4/3] overflow-hidden bg-muted">
                  {fotosMap[i.id] ? (
                    <img src={fotosMap[i.id]} alt={i.titulo} loading="lazy" className="h-full w-full object-cover transition group-hover:scale-105" />
                  ) : (
                ...
                <div className="p-4">
                  <div className="mb-1 flex items-center gap-2">
                    <Badge variant="secondary">{FINALIDADE_LABEL[i.finalidade] ?? i.finalidade}</Badge>
                  </div>
                  <h3 className="line-clamp-1 font-semibold">{i.titulo}</h3>
                  <p className="mt-1 flex items-center gap-1 text-xs text-muted-foreground"><MapPin className="h-3 w-3" /> {[i.endereco_bairro, i.endereco_cidade].filter(Boolean).join(", ") || "—"}</p>
      ...
      {sobre && (
        <section id="sobre" className="border-t border-border bg-muted/30">
          <div className="mx-auto max-w-3xl px-6 py-16">
            ...
            <article className="prose prose-sm max-w-none [&_h2]:mt-6 [&_h2]:text-lg [&_h2]:font-semibold [&_p]:mb-3 [&_ul]:list-disc [&_ul]:pl-6" dangerouslySetInnerHTML={{ __html: sobre }} />
          </div>
        </section>
      )}
        ...
        <form onSubmit={submit} className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <Input value={form.nome} onChange={(e) => setForm({ ...form, nome: e.target.value })} maxLength={200} required />
            </div>
            <div>
              <Label className="mb-1.5 block text-xs uppercase text-muted-foreground">Telefone</Label>
              <Input value={form.telefone} onChange={(e) => setForm({ ...form, telefone: e.target.value })} maxLength={40} />
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <Textarea rows={4} value={form.mensagem} onChange={(e) => setForm({ ...form, mensagem: e.target.value })} maxLength={2000} />
          </div>
...
function NotPublished() {
  return (
    <div className="flex min-h-screen flex-col items-center justify-center gap-3 text-center">
      ...
      <p className="max-w-md text-sm text-muted-foreground">Esta imobiliária ainda não publicou seu site público.</p>
